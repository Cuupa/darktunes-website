...
- **Accounting** *(admin-only)*: `/admin/accounting` — SOS Generator (upload distributor CSVs, process statements), **Save to Portal** (persists territory metrics, merch orders, period summaries, event/promo impact correlations), and statement history.

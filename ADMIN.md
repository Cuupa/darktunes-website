...
- **System** *(admin-only)*: `/admin/system` — Health dashboard (queue stats, DB connectivity), Audit Log (`sync_logs`), Error Log (failed sync runs), App Errors (`app_logs`), and Maintenance tasks (clear logs, purge orphaned releases, reset checklists, manage accreditations, clear stats, **audited SOS data purge**: failed bronze / all bronze archives / portal gold analytics). Supports full-text search, source/status filters, and pagination.

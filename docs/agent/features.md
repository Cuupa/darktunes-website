...
**Admin accounting (`/admin/accounting`):** Opens the workspace tabs directly (Files first). No Assistant / Quick Start / Advanced chooser. Period month fields live on Files. Billing path: **Files → Amounts → Statements** (SEPA on Statements) → **Rules**. **Insights** (archives, portal persist, trends) sits last, off the money path. `?subTab=payout` and `?guidedStep=settle` open Statements; `?subTab=trends` opens Insights. Preview/Excel work without a complete bronze archive; creating a statement draft does not. Statements + SEPA open from the database when a period is set — no CSV session required. Replacing a session file re-parses it (same id) and clears `bronzeBatchId` until re-archive. A parse error does not stall other files. Dirty period settings are saved before switching period (previous period key is stored synchronously). Worker process results/errors are sequence-guarded. Operator toasts use `explainSosError`: cause in plain language, no storage/HTTP jargon.

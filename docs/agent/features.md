...
**Excel worker hardening:** The worker emits `excel-progress` phases `original-reports` → `summary` (the latter right before ExcelJS `writeBuffer()`, which cannot report progress). ExcelJS `writeBuffer()` returns a `Buffer`/`Uint8Array`; `toTransferableArrayBuffer()` copies the bytes into a real `ArrayBuffer` before `postMessage(..., [buffer])`. Typed failures (`ExcelExportWorkerError` codes `EXCEL_TIMEOUT`, `EXCEL_WORKER_NOT_READY`, `EXCEL_WORKER_DATA_MISSING`, `EXCEL_ARTIST_NOT_IN_WORKER`, `EXCEL_RAW_ROWS_LIMIT`) map to specific toasts. Raw exports above `EXCEL_MAX_TOTAL_RAW_ROWS` (1.5 M across tabs) fail closed with the count and limit. Reporting export buttons + the Excel dialog are disabled while `isProcessing || excelBusy`; after a timeout the request is orphaned and `excelBusy` stays true until the worker answers. ZIP-of-all forwards per-artist Excel progress and reports skipped artists (placeholder file per artist instead of a silent gap).

...
**SOS ingest progress:** File cards show reading (bytes), decoding, CSV structure, then `row x of y`. Parse runs **once in the worker** (main thread only reads/decodes). A pipeline banner covers aggregation. Bronze archive errors surface the API message. Large files (>50 MB) warn that parsing can take minutes.
...
**7-step workflow:** review → draft upload → label approve → artist viewed → invoice created → received → paid. The UI stepper is derived KPIs (`statementWorkflow.ts`). **DAL enforces FROM→TO** via `STATEMENT_TRANSITIONS` in `statementStatusTransitions.ts` (`updateSalesStatementStatus` throws `InvalidStatementTransitionError` → HTTP 422). Invoice from `label_approved` (skip notified/viewed) stays allowed. Payment from invoice `sent` stays allowed (sets `received_at` if empty). No unlock / unpay in the app — reverse status only via support SQL. Known gaps tracked by the contract: `recordStatementView` bypasses the transition assert (#622); a failed approval email strands the statement at `label_approved` with no retry writer (#623); invoice `sent` is currently set before the mail result (#621).

...
**Queue:** `sync_queue` DAL in `syncQueue.ts` — statuses: `pending` | `running` | `done` | `failed` | `cancelled`. Job types: `full`, `spotify`, `discogs`, `youtube` (legacy), `odesli`. Executor `/api/sync` (~280s budget, single-flight lease with owner token): claims jobs only with ≥50s headroom (no mid-kill zombies), paces ~400ms between artists, **self-chains** another `/api/sync` after lease release while due pending remain (one logical drain across Vercel slices). Rate-limited artists reschedule with cooldown; other artists keep draining. Stuck `running` lock 6m; `getSyncQueueStats` recovers zombies. **Admin control (Advanced only):** `GET/POST /api/admin/sync/jobs` — list, cancel, retry. **Admin UI rule:** no infra setup copy. Odesli: never retry 429 in-request — reschedule with cooldown.

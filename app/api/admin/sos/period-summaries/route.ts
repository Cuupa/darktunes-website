...
export const GET = withErrorHandler(async (): Promise<NextResponse> => {
  const supabase = await requireAccountingRole()
  const { data, error } = await supabase
    .from('sos_period_summaries')
    .select('id, period_start, period_end, total_revenue, total_payout, artist_count, artist_breakdowns, platform_breakdowns, source_batch_ids, created_at')
...
export const POST = withErrorHandler(async (req: NextRequest): Promise<NextResponse> => {
  ...
  const {
    ...
    artist_count, artist_breakdowns, platform_breakdowns, source_batch_ids,
  } = body as {

...
- **Cron endpoint auth** (all sync routes: `/api/sync`, `/api/sync/queue`, `/api/sync/execute`, `/api/sync-youtube`, `/api/sync-api`) — Vercel cron calls (`x-vercel-cron: 1`) must always present `Authorization: Bearer <CRON_SECRET>`; `CRON_SECRET` is mandatory; a missing or wrong secret is rejected with HTTP 401. This prevents unauthenticated callers from triggering resource-intensive sync jobs. Admin/user JWTs (validated via `supabase.auth.getUser()`) are still accepted for manual triggers.

...
- [ ] Excel export on a large artist: toast shows “Collecting original reports…” then “Writing summary workbook…” (not stuck on the title); PDF/Excel/Export All buttons are disabled while CSV processing or an Excel build runs; an export over 1.5 M raw rows fails with the row count and limit; a >5 min export shows the timeout message (not “original tabs could not be attached”)

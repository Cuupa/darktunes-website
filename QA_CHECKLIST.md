...
- [ ] Excel export on a large artist: toast shows “Collecting original reports…” then “Writing summary workbook…” (not stuck on the title); PDF/Excel/Export All buttons are disabled while CSV processing or an Excel build runs; an export over 1.5 M raw rows opens the fallback dialog with the row count and limit; a >5 min export opens the fallback with the timeout reason (not “original tabs could not be attached”)
- [ ] Excel raw export after the worker lost its data (page reload or worker restart): progress shows “Rebuilding original reports…” and the export retries once automatically; if it still fails, the fallback dialog opens with the reason. **Download summary only** saves `*_summary-only.xlsx` and warns; **Cancel** downloads nothing; a cancelled export does not open the dialog.
- [ ] Excel while CSVs re-process: no file downloads; toast says files/rules changed and to export again. A second Excel click while one is running says wait. Raw-on with no worker does not download a summary-only file.
- [ ] ZIP-of-all with a skipped raw Excel finishes and shows a warning naming the skipped artist count (plus the placeholder file with the error reason). Cancelling / starting another ZIP does not download the first ZIP.

...
- **Accounting tabs follow the billing path:** Files, Amounts, Statements (with SEPA), Rules, then Insights. Portal Data and Trends are no longer separate stops in the money flow. `?subTab=payout` opens Statements; `?subTab=trends` opens Insights.
